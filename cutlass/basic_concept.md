...
#### 相关具体实现
 ...
 - ceil_div: 二元函数, 进行维度向上取整的除法操作。
  ...
  - 如果左操作数是Tuple，右操作数是Integral，首元素与右操作数进行Integral的除法操作;（浮点数的处理逻辑比较复杂，集中在fold的处理。）
 ...
 - shape_dive: 二元函数, 对应位置元素进行左操作数对右操作数进行向上取整处理。代码处理与ceil_div基本一致。
  ...
  - example:
   ...
   tuple<tuple<int, _6>, tuple<_3, _3>, int> test_right(tuple<int, _6>(2, _6{}),
                                                       tuple<_3, _3>(_3{}, _3{}), 2);
   print(shape_div(test, 3));           // ((1,6),(4,3),4)
